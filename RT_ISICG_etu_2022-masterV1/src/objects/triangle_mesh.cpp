...
namespace RT_ISICG
{
	...
	{
		return _intersectBVH( p_ray, p_tMin, p_tMax, p_hitRecord );
	}
	...
	bool MeshTriangle::_intersectBVH(	const Ray& p_ray,
										const float p_tMin,
										const float p_tMax, 
										HitRecord& p_hitRecord) const 
	{
		...
		else if ( _bvh->intersect( p_ray, p_tMin, p_tMax, p_hitRecord ) )
		{
			p_hitRecord._object = this;
			return true;
		}
